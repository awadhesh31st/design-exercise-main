...
function AvatarImage({ className, onLoadingStatusChange, ...props }: AvatarImageProps) {
  ...
  return <img data-slot="avatar-image" className={cn('aspect-square size-full', className)} onLoad={handleLoad} onError={handleError} {...props} />
}
...
function AvatarFallback({ className, delayMs = 0, children, ...props }: AvatarFallbackProps) {
  ...
  return (
    <div data-slot="avatar-fallback" className={cn('bg-muted flex size-full items-center justify-center rounded-full', className)} {...props}>
      {children}
    </div>
  )
}
